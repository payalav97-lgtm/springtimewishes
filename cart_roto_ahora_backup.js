...
// STW fix: make top CART label clickable without touching layout
document.addEventListener("click", function (event) {
  if (event.target.closest(".cart-drawer, .cart-overlay")) return;
  ...
  while (el && el !== document.body) {
    ...
    if (label.startsWith("cart /") || label === "cart") {
      event.preventDefault();
      event.stopPropagation();
      event.stopImmediatePropagation();

      if (window.STWCartOpen) {
        window.STWCartOpen();
      }
...
/* STW cart corner click fix — no layout / no HTML changes */
(function () {
  if (window.__STW_CART_CORNER_CLICK_FIX__) return;
  window.__STW_CART_CORNER_CLICK_FIX__ = true;

  function openCartHard() {
    if (typeof window.STWCartOpen === "function") {
      window.STWCartOpen();
      return;
    }

    const overlay = document.querySelector(".cart-overlay, [data-cart-overlay]");
    const drawer = document.querySelector(".cart-drawer, [data-cart-drawer], #cartDrawer");

    if (overlay) overlay.classList.add("is-open", "open", "active");
  ...
  function looksLikeCartLabel(el) {
    while (el && el !== document.body) {
      ...
      if (txt === "cart" || txt.startsWith("cart /") || txt.startsWith("cart/")) {
        return true;
      }
  ...
  window.addEventListener("click", function (event) {
    ...
    const clickedCartText = looksLikeCartLabel(event.target);
    const clickedCartCorner = y <= 95 && x >= window.innerWidth - 230;

    if (!clickedCartText && !clickedCartCorner) return;
